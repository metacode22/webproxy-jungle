...
void doit(int connfd) {
  ...
  // Cache에서 못 찾았으니 End Server로 연결하는 부분이다.
  // 이 때 hostname과 port는 client의 hostname과 port이다. 익명성을 보장해주는 기능은 없는 것 같다.
  ...
  while ((n=Rio_readlineb(&server_rio, buf, MAXLINE)) != 0) {
    ...
    if (sizebuf < MAX_OBJECT_SIZE) // 작으면 response 내용을 적어놈. 캐쉬에 넣을 수 있다는 뜻인가?
...
void build_http_header(char *http_header, char *hostname, char *path, int port, rio_t *client_rio) {
  ...
  while (Rio_readlineb(client_rio, buf, MAXLINE) > 0) {
    ...
    // 대소문자 구분 없이 비교하는데, strlen(x), 즉 x바이트만큼만 비교를 해라?
